...
data %>%
  ...
  labs(
    title = "Figure 1: Number of packages updated per year", 
    x = "Year", 
...
data %>%
  ...
  theme() +
  labs(
    title = "Figure 2: Breakdown of types of versions between December 2010 and 2020", 
    x = "Version type", 
...
With an average of 675 stable updates (major, minor and patches) annually since 2015 to packages used in [Tracker project](https://github.com/canada-ca/tracker), once in production, the project will require maintenance to keep the dependancies up to date.
```{r, echo=FALSE}
average_table <- stable_versions %>%
  filter(date >= "2015-01-01") %>%
  group_by(year) %>%
  summarise("number of stable versions" = n())
```
...
```{r, include=FALSE}
mean(average_table$`number of stable versions`)
```
...
stable_versions %>%
  ...
  group_by(version_type) %>%
  summarise(n = n()) %>%
  mutate(annual_average = n / 6) %>%
  mutate(monthly_average = n / (6 * 12)) %>%
  mutate(weekly_average = n / (6 * 52))
```
...
scale_x_continuous(name="Weeks since last update", breaks = seq(min(versions_in_a_year$weeks), max(versions_in_a_year$weeks), by = 4)) +  labs(y="Expected number new versions since last update")
