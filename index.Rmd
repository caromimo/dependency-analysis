...
```
Based on those averages, and assuming that they are representative of the years to come, once in production, the [Tracker project](https://github.com/canada-ca/tracker/) will require maintenance and continued development as new versions will continue to be made available. **Figure 4** illustrates the number of new patches, minor versions, major versions and their total through time.
```{r, include=FALSE}
week_number <- c(0:52)
...
ggplot(long, aes(x = week_number, y = value)) + 
  ...
  labs(
    title = "\nFigure 4: Expected number of new versions over a year", 
