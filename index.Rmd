...
This project explores the number and frequency of updates in a project currently under development. It aims to shed light on the update requirements once the project will be in production. We used the [dependency manifest file](https://github.com/canada-ca/tracker/blob/003a3d6e0452fb9a248a7d51fd5556bc03dfbcc6/frontend/package.json) from the frontend of Government of Canada's Tracker project as a representative example of a modern software project. Method and steps to gather and clean the data are described at the end of this post.
...
**Figure 7** demonstrates the differences between 
The distributions (Figure 7, the number of versions per year and the average time between major versions of days between major versions changeaverage time between major versions 
...
```{r, echo=FALSE}
major_versions_in_2018 <- interval_between_major_versions_only %>%
  filter(date >= "2018-01-01") %>%
  filter(date < "2019-01-01")
```

```{r, echo=FALSE}
positions <- c(0.25, -0.25, 1.25, -1.25, 0.5, -0.5, 1.5, -1.5, 0.75, -0.75, 1.75, -1.75, 2, -2)
directions <- c(1, -1)
...
#### PLOT ####
timeline_plot <- df %>% ggplot(
  aes(x=date, y=0, label=major_version)
  ) +
  theme_classic() +
  geom_hline(
    yintercept=0,
  ...
  geom_text(
    data=year_df, 
    aes(x=year_date_range,y=2,label=year_format, fontface="bold"),
    size=5.5,
    color='black'
    ) +
  geom_text(
    aes(y=text_position,label=major_version),
    size=2.5
    )
...
```{r, echo=FALSE}
major_versions_in_2019 <- interval_between_major_versions_only %>%
  filter(date >= "2019-01-01") %>%
  filter(date < "2020-01-01")
```
...
interval_between_major_versions_only %>% 
  ...
  ggplot(aes(x=date)) +
    ...
    labs(
      title = "Figure X: Major versions per date in 2019",
      x = "Date in 2019",
      y = "Count"
    )
```
...
interval_between_major_versions_only %>% 
  ...
  ggplot(aes(x=date)) +
    geom_bar(colour = "black", fill = "white") + 
    labs(
      title = "Figure X: Major versions per date in 2018",
      x = "Date in 2018",
      y = "Count"
    )
```
