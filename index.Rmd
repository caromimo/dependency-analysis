...
In this project, I explored the number and frequency of updates in a project currently under development to characterize the update requirements once the project will be in production. I used the [dependency manifest file](https://github.com/canada-ca/tracker/blob/003a3d6e0452fb9a248a7d51fd5556bc03dfbcc6/frontend/package.json) from the frontend of Government of Canada's Tracker project as a representative example of a modern software project. the methods and steps to gather and clean the data are described at the end of this post in the Methods section.
...
data <- data %>%
  mutate(
    version_type = if_else(prerelease != "", "pre-release", 
                           if_else(patch == "0", "patch",
                                   if_else(minor == "0", "minor", "major"))) 
  )
```
...
<!-- data <- data %>% -->
<!--   mutate( -->
<!--     version_type = if(prerelease != "", "pre-release") -->
<!--     version_type = if(patch == "0", "patch", -->
<!--                                    if_else(minor == "0", "minor", "major")))  -->
<!--   ) -->
<!-- ``` -->
...
versions_in_a_year <- weeks %>% mutate(weeks = weeks, number_major_versions = weeks * 0.875, number_mmp = weeks * 14.029)

versions_in_a_year %>% ggplot(aes(x=weeks, y=number_major_versions)) + geom_point() + labs(y="Expected number of new major versions since last update") + scale_x_continuous(name="Weeks since last major version was updated", breaks = seq(min(major_versions_in_a_year$weeks), max(major_versions_in_a_year$weeks), by = 4))

versions_in_a_year %>% ggplot(aes(x=weeks, y=number_major_versions)) + geom_point() + labs(y="Expected number of new major versions since last update") + scale_x_continuous(name="Weeks since last major version was updated", breaks = seq(min(versions_in_a_year$weeks), max(versions_in_a_year$weeks), by = 4))

ggplot(versions_in_a_year, aes(x=weeks, y = value, color = variable)) + 
    geom_line(aes(y = number_major_versions, col = "major versions only")) + 
    geom_line(aes(y = number_mmp, col = "major, minor, patches")) + 
    scale_x_continuous(name="Weeks since last update", breaks = seq(min(versions_in_a_year$weeks), max(versions_in_a_year$weeks), by = 4)) + 
    labs(y="Expected number new versions since last update") 
...
**Figure 1** is a plot of the number of versions for each package since December 29th, 2010 ordered by number of versions. The package with the most versions is at the top (webpack had 307 versions) and the package with the least versions is at the bottom (webpack-config-utils had six versions). **Figure 1** demonstrates the variability in the number of versions among packages. Note that not all packages existed in December 2010 and the creation date of packages might account for some of this variability.
