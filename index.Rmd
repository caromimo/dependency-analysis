...
Finally, the dataset contained 12 columns as follow:
```{r}
colnames(data)
```
...
The dependency manifest file uses a total of 72 packages, which, all together, were updated through major, minor or patches in a total of 7,322 versions between December 29th 2010 to November 12th, 2020. 
...
Figure 2 is a heatmap of the number of versions per year per package presented in alphabetical order. The more versions in a year for a given package, the darker the color. This figure illustrates that packages have been created in most years and that the number of version per package per year is highly variable. Some packages with most of their versions in the early years of their creation (e.g., babel-core), others with most versions in recent years (e.g., react) and others without clear trends (e.g., d3). Note that at the time of writing this, the year 2020 was not complete.  
