...
Using the following [Bash](https://www.gnu.org/software/bash/) command, package names (both development and production) were extracted from the `package.json` and details for each was retrieved from the npm API.

The command uses [jq](https://stedolan.github.io/jq) to select dependency name from the json and format the API response as CSV.  [curl](https://curl.se/) was used to retrieve data from the API.
```
...
The resulting csv contained three columns; the name of the package, the version and a time stamp.
...
```{r}
library(here)
packages <- read_csv(here("data", "raw", "nodejs.csv"), col_names = c("package_name", "version", "time_stamp"))
```
